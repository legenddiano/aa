...
void ATacticalStrikeCharacter::Tick(float DeltaSeconds) { Super::Tick(DeltaSeconds); }
...
{
    const int32 Loaded = FMath::Min(MagazineSize - AmmoInMagazine, ReserveAmmo);
    AmmoInMagazine += Loaded; ReserveAmmo -= Loaded;
}
void ATacticalStrikeCharacter::Fire()
{
    if (bSprinting || AmmoInMagazine <= 0 || GetWorld()->GetTimeSeconds() < NextFireTime) return;
    NextFireTime = GetWorld()->GetTimeSeconds() + FireRate; --AmmoInMagazine;
    FVector Start; FRotator Rot; GetController()->GetPlayerViewPoint(Start, Rot);
    const FVector End = Start + Rot.Vector() * 10000.0f; FHitResult Hit;
